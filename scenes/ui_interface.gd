...
const Menu2 := preload("res://scenes/option1.tscn")

# Declaración de nodos y variables
#onready var tween := $Tween
onready var screens_container := $VBoxContainer
onready var btn_play := $VBoxContainer/btn_play
onready var close_button = $VBoxContainer/btn_salir
...
func _ready() -> void:
	...
	btn_play.connect("pressed", self, "mostrar",[Menu2])
	close_button.connect("pressed", self, "cerrar_escena")
